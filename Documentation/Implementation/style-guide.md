...
| `--feedback-checkbox-border-color` | `colors.feedback.checkboxBorder` | `Color?` | `nil` (adaptive: `white @ 28%` in dark, `black @ 35%` in light) | Checkbox unchecked outline; also applied to the notes editor outline in modal mode. Set explicitly when `--feedback-sheet-background-color` is pinned, as the adaptive fallback tracks device interface style. |
